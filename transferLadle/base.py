...
def transfer(cfg_):
    ...
    data['portal_id'] = cfg.get('transfer', 'portal_id')
    data['pswd'] = cfg.get('transfer', 'pswd')

    session.post('https://auth.sb69gbmm.com/1/sbrPortal/transferNew', data = data, timeout = 9)
    while (True):
        try:
            session.post('https://auth.sb69gbmm.com/1/sbrPortal/transferNew', data = data, timeout = 9)
        except:
